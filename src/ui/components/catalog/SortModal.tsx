...
export default function SortModal() {
  ...
  return (
    <section
      className={`fixed max-md:inset-x-0 md:inset-y-0 max-md:bottom-0 md:right-0 bg-background h-fit md:h-full w-full md:w-fluid-width-xxs before:absolute before:left-0 max-md:before:right-0 max-md:before:top-0 md:before:inset-y-0 max-md:before:h-px md:before:w-px before:bg-foreground max-md:pb-14 p-1 ${isSortCollapsed ? "max-md:translate-y-full md:translate-x-full" : "translate-0"} transition-transform duration-300 ease-cubic`}>
      <header className="flex items-center justify-between relative after:hidden md:after:block after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-foreground-muted px-4 py-2">
        ...
        <button
          ...
          onClick={() => setIsSortCollapsed(true)}
