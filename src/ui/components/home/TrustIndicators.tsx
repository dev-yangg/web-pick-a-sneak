    ...
    <section className="bg-foreground text-background grid place-items-center">
      <div
        ...
        {trustIndicators.map(({ label, description, icon: Icon }, index) => (
          <article
            key={label}
            className={`flex max-md:grow max-md:shrink max-md:basis-[min(380px,100%)] md:w-full items-center gap-4 py-4 md:py-8 px-4 lg:px-10 $  ${index === 1 && "md:relative before:left-0 [&::before,&::after]:text-background/45 [&::before,&::after]:h-[80%] [&::before,&::after]:vertical-divider-line-pseudo after:right-0"}`}>
            <span
              aria-hidden="true"
